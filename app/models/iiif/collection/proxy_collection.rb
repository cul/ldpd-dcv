class Iiif::Collection::ProxyCollection < Iiif::BaseResource
  ...
  def proxied_items(part_of_json, container_proxy_id)
    ...
    children.map do |child_doc|
      ...
      if child_doc['type_ssim']&.include?(Iiif::Collection::COLLECTION_PROXY_TYPE)
        # do collection
        child_id = subcollection_id(child_doc['proxyFor_ssi'])
        Iiif::Collection::ProxyCollection.new(id: child_id, solr_document: solr_document, children_service: children_service, route_helper: route_helper, ability_helper: ability_helper).tap {|c| c.part_of_json = part_of_json}
      else
        # do manifest
  ...
  def part_of
    ...
    @part_of_json ||= [
      Iiif::Collection::ProxyCollection.new(
  ...
  def subcollection_id(container_id)
    ...
    collection_id = route_helper.iiif_proxy_collection_url(collection_params)
  end
  ...
  def manifest_id(child_doc)
    ...
    route_helper.iiif_proxy_collected_manifest_url(routing_params)    
  end
