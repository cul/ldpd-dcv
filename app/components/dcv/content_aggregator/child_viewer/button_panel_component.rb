...
module Dcv::ContentAggregator::ChildViewer
  class ButtonPanelComponent < ViewComponent::Base
    ...
    def show_zoom?
      has_zoom? && (has_viewable_children?(@document) || archive_org_id_for_document(@document) || @document.resource_result?)
    end
    ...
    def has_chapters?
      child_datastreams.include?('chapters')
    end

    def has_synch?
      child_datastreams.include?('synchronized_transcript')
    end
    ...
    def child_datastreams
      @child.fetch(:datastreams_ssim, [])
    end
  end
end