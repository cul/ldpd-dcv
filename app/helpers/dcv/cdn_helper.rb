module Dcv::CdnHelper
  ...
  def get_manifest_url(document, collection: nil, **options)
    ...
    return iiif_proxy_collection_url(collection_registrant: registrant, collection_doi: doi) if collection
  ...
  def thumbnail_url(document, options = {})
    ...
    if schema_image.present?
      Dcv::Utils::CdnUtils.asset_url(base_type: 'featured', id: schema_image.split('/')[-1], size: size, region: 'full', format: 'jpg')
      ...
      Dcv::Utils::CdnUtils.asset_url(base_type: 'featured', id: document.id, size: size, region: 'full', format: 'jpg')
    end
  end
  ...
  def poster_url(item, opts = {})
    ...
    opts = { base_type: 'standard', size: 768, region: 'full', format: 'jpg' }.merge(opts)
  ...
  def thumbnail_for_site(site)
    ...
    if id
      get_asset_url(id: id, size: 256, base_type: 'featured', region: 'full', format: 'jpg')
    else
