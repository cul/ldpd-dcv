  ...
  if($('.download-button').length > 0) {
    ...
    $('#dcvModal').on('shown.bs.modal', function (event) {
      ...
      if(typeof(downloadButton.attr('data-download-content-url')) != 'undefined' && downloadButton.attr('data-download-content-url').length > 0) {
        downloadsList.html('<li class="dropdown-item"><a class="dropdown-link" href="' + downloadButton.attr('data-download-content-url') + '" target="_blank" onclick="$(\'#dcvModal\').modal(\'hide\')"><span class="fa fa-download"></span> Download File</a></li>');
      } else {
