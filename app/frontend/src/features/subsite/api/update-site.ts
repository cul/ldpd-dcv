...
const updateSite = async (
  site: Site | SiteGeneralProperties | SitePortraitImageUris | SiteNavGroups,
) => {
  if (!site.slug)
    throw Error(
      'Error: An API request was made that required a subsite slug as identifier but was not given one.',
    );
