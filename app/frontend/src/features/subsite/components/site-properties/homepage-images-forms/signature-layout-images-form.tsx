...
const schema = z.object({
  banner: z
    .instanceof(FileList)
    .refine(
      (files) => files.length === 0 || (files.length === 1 && files[0].size <= 5 * 1024 * 1024),
    )
    .refine(
      (files) => files.length === 0 || files[0]?.name.split('.').pop()?.toLowerCase() === 'png',
  ...
  watermark: z
    ...
    .refine(
      (files) => files.length === 0 || files[0]?.name.split('.').pop()?.toLowerCase() === 'svg',
...
const SignatureLayoutImagesForm = ({ slug }: { slug: string }) => {
  ...
  const submitHandler = async (data: FormInput) => {
    try {
      ...
      if (data?.banner?.length === 0 && data?.watermark?.length === 0)
        throw Error('You have not uploaded any images.');
      ...
      if (error instanceof Error) {
        setSubmissionAlert({
          isError: true,
          msg: `There was an error uploading the image: ${error?.message}`,
        });
      } else {
        setSubmissionAlert({
          isError: true,
          msg: 'An unknown error occurred while uploading your image. Please try again.',
        });
      }
      setShowAlert(true);
    }
      ...
      <Form onSubmit={handleSubmit(submitHandler)}>
        <Stack gap={4}>
          <Form.Group controlId="SignatureLayoutFormBanner" className="d-flex flex-column">
            ...
            <div>
              <span className="fw-bold">Banner image: </span>
              {site.hasBannerImage ? getFilenameFromUrl(site.bannerImageUrl) : 'using default.'}
            </div>
            <Form.Text>
              {site.hasBannerImage
                ? ''
                : 'You have not uploaded a banner image yet. If you choose the signature layout, the default image will be used.'}
            </Form.Text>
          ...
          <Form.Group controlId="SignatureLayoutFormWatermark" className="d-flex flex-column">
            ...
            <div>
              <span className="fw-bold">Watermark image: </span>
              {site.hasWatermarkImage
                ? getFilenameFromUrl(site.watermarkImageUrl)
                : 'using default.'}
            </div>
            <Form.Text>
              {site.hasWatermarkImage
                ? ''
                : 'You have not uploaded a watermark image yet. If you choose the signature layout, the default image will be used.'}
            </Form.Text>
            <ImageUploadPreview slug={slug} type="watermark" />
            <Form.Control type="file" {...register('watermark')} />
            {errors && errors.watermark && (
              <Form.Text className="text-danger">{errors.watermark.message}</Form.Text>
            )}
            {site.hasWatermarkImage && (
              <Button variant="danger" onClick={deleteWatermarkHandler}>
                Delete Watermark Image (and use default)
              </Button>
            )}
          </Form.Group>
