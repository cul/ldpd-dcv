...
const SitePagesGeneralFormRow = ({
  slug,
  field,
  index,
  register,
  remove,
}: SitePagesGeneralFormRowProps) => {
  return (
    <div
      key={field.id}
    ...
    >
      <Row key={field.id}>
        ...
        <Col xs={3} md={3}>
          ...
          <Row className="px-3 fst-italic text-muted" style={{ fontSize: '0.850rem' }}>
            Last updated: {formatDateRelative(field.updatedAt)}
          </Row>
        </Col>
        <Col xs={3} md={2}>
          {field.pageSlug !== 'home' && (
