...
describe NavLink do
	...
	let(:about_link) { FactoryBot.create(:nav_link, external: false, sort_label: "00:#{about_label}", link: 'about') }
	describe '#label' do
	...
	describe '#about_link?' do
		...
		context "external links are not about link" do
			subject { FactoryBot.create(:nav_link, external: true, sort_label: "00:#{about_label}", link: 'https://about.org') }
			it { is_expected.not_to be_about_link }
		end
		context "internal links to other pages are not about link" do
			subject { FactoryBot.create(:nav_link, external: false, sort_label: "00:#{about_label}", link: 'not_about') }
			it { is_expected.not_to be_about_link }
		end
	end
end