...
describe Resolve::CatalogController, type: :controller do
	before do
	...
	let(:known_id) { 'donotuse:1234' }
	let(:legacy_id) { 'rbml_css_0055' }
	let(:known_doi) { '10.1234/567-abcd' }
	let(:solr_doc) { SolrDocument.new(site.default_filters.merge(id: known_id, identifier_ssim: [legacy_id, known_id], ezid_doi_ssim: ["doi:#{known_doi}"])) }

	describe '#doc_url_in_site_context' do
		context 'nil site' do
			let(:expected_url) { "http://test.host/catalog/#{known_doi}" }
		...
		context 'site with local search' do
			...
			let(:expected_url) { "http://test.host/dlc_site/#{known_doi}" }
		...
		context 'site with custom search' do
			...
			let(:expected_url) { "http://test.host/#{template_defined_slug}/#{known_id}" }
	...
	describe '#show' do
		...
		before do
			...
			allow(controller).to receive(:params).and_return({id: legacy_id})
		...
		context 'for a non-existent document' do
			...
			it 'redirects to resolved url' do
				expect { controller.show }.to raise_error Blacklight::Exceptions::RecordNotFound
			end
		end
	end
end
