...
describe Cul::Hydra::Datastreams::RelsInt , type: :unit do
  ...
  let(:profile) do
    profile_xml = fixture(File.join('rels_int', 'rels_int_profile.xml')).read
  ...
  context "on a new object" do
    ...
    it "should serialize to appropriate RDF-XML" do
      blank_relsint = fixture(File.join('rels_int', 'rels_int_blank.xml')).read
  ...
  it "should propagate relationship changes to the appropriate graph in RELS-INT" do
    ...
    datastream.add_relationship(rels_ext,:source, "FOO", true)
    datastream.add_relationship(datastream,:source, "BAR", true)
  ...
  it "should clear matching relationships selectively" do
    datastream.content=test_relsint
    rels_ext = ActiveFedora::Datastream.new(inner,"RELS-EXT")
    test_pred = 'http://purl.org/dc/terms/source'
    expect(datastream.relationships(rels_ext,:source)).to_not be_empty
    datastream.clear_relationship('info:fedora/test:relsint/RELS-EXT',:source)
    expect(datastream.relationships(rels_ext,:source)).to be_empty
    expect(datastream.relationships(datastream,:source)).to_not be_empty
  end
