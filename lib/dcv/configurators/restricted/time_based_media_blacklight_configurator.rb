class Dcv::Configurators::Restricted::TimeBasedMediaBlacklightConfigurator
  ...
  def self.configure(config)
    ...
    config.show.route = { controller: 'restricted/time_based_media' }

    config.default_solr_params = {
      :fq => [
        'object_state_ssi:A', # Active items only
        'active_fedora_model_ssi:(ContentAggregator OR GenericResource)'
      ],
      :qt => 'search'
    }
