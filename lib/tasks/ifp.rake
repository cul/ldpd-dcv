namespace :util do
  namespace :ifp do
    ...
    task composite: :environment do
      open('tmp/ldpd_499461.txt') do |blob|
        blob.each do |pid|
          pid.strip!
          puts "#{pid} into composite"
          obj = ActiveFedora::Base.find(pid)
          obj.clear_relationship(:is_constituent_of)
          obj.add_relationship(:is_constituent_of, 'info:fedora/cul:7d7wm37q33')
          obj.clear_relationship(:publisher)
          ...
          obj.add_relationship(:publisher, 'info:fedora/cul:xwdbrv15p4') # private publish target
          obj.clear_relationship(:contributor)
          obj.add_relationship(:contributor, 'Vietnam', true)
          obj.add_relationship(:cul_member_of,'info:fedora/ldpd:499461')
          obj.datastreams['RELS-EXT'].content_will_change!
          obj.save
        end
      end
    end
