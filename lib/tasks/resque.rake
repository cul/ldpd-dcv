...
namespace :resque do
  ...
  task :start_workers => :environment do
    run_workers(Dcv::Queue::QUEUES_IN_DESCENDING_PRIORITY_ORDER, RESQUE_CONFIG['workers'] || 1)
  end
  ...
  def run_workers(queue_names, count = 1)
    ...
    env_vars = {
      ...
      'TERM_CHILD' => '1',
      'INTERVAL' => '0.1' # The default interval is 5 (seconds), but we expect to send a lot of quick reindexing jobs to DLC, so we need a more frequent job polling frequency per worker
    }
